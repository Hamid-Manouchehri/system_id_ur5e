function [q1, q2, q3, q4, q5, q6, dq1, dq2, dq3, dq4, dq5, dq6, ...
    ...
    for k = 1:6
        ...
        if (k==1)
            w(:,k) = R' * (w_0 + traj(6+k)*z0);  % angular velocity
            wdot(:,k) = R' * ( wdot_0 + traj(12+k)*z0 + traj(6+k)*cross(wdot_0,z0) );  % angular acceleration
            pddot(:,k) = R'*pddot_0 + cross(wdot(:,k),r) + cross(w(:,k), cross(w(:,k),r) );  % linear acceleration
        else
